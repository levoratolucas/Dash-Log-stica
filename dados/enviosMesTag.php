...
$query2 = "select plp.plp as Tag,count(plp.objeto) as Envios,date_format(plp.dt_envio,'%y-%m') as Data 
from tb_csv_plp as plp
JOIN spree_stores_on ss on 
ss.store_id = plp.store_id
...
GROUP by date_format(plp.dt_envio,'%y-%m'),plp.plp
order by count(plp.objeto) desc ";

$meses = queryArray($db_apisup, $query, 'data');
$queryTag = "select plp.plp
from tb_csv_plp as plp
JOIN spree_stores_on ss on 
ss.store_id = plp.store_id
where 1=1 ".$loja."
GROUP by plp.plp
order by plp.plp desc ";
$tags =  queryArray($db_apisup, $queryTag, 'plp');
?>
<style>
  .search2 {
    display: flex;
...
<div class="search2">
  <form id="searchForm2" method="get" action="" style="margin-bottom: 10px; display:flex;">
    ...
    <div class="search2">
      ...
      <?php
      echo '<option value="1">RESET TAG</option>';
      foreach ($tags as $tag) {
